...
namespace OxyPlot
{
    ...
    public class Conrec
    {
        ...
        /// The renderer.
        /// </param>
        public static IDictionary<double, IList<Controller.DataPointBase>> Contour( double[,] d, double[] x, double[] y, double[] z )//, RendererDelegate renderer )
        {
            //IList<Controller.DataPointBase> list = new List<Controller.DataPointBase>();
            IDictionary<double, IList<Controller.DataPointBase>> dict = new Dictionary<double, IList<Controller.DataPointBase>>();
            IList<Controller.DataPointBase> list0 = null;
                            ...
                            if( z[k] >= dmin && z[k] <= dmax )
                            {
                                ...
                                for( m = 1; m <= 4; m++ )
                                {
                                    ...
                                    if( caseValue != 0 )
                                    {
                                        ...
                                        if( z[k] != 0 )
                                        {
                                            if( dict.ContainsKey( z[k] ) )
                                            {
                                                list0 = dict[z[k]];
                                            }
                                            else
                                            {
                                                list0 = new List<Controller.DataPointBase>();
                                                dict.Add( z[k], list0 );
                                            }

                                            //Controller.DataPointBase dataPoint = new Controller.DataPointBase( x2, y2, z[k] );
                                            list0.Add( new Controller.DataPointBase( x2, y2, z[k] ) );

                                        }
                                        //System.Diagnostics.Debug.WriteLine( string.Format( "{0} {1} {2} {3} {4}", x1, y1, x2, y2, z[k] ) );
